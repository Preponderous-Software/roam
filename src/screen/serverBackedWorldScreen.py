...
class ServerBackedWorldScreen:
    ...
    def __init__(
        ...
        logger.info(
            f"Initializing ServerBackedWorldScreen with session_id: {session_id}"
        )
        ...
        self.mouse_button_held = {
            1: False,
            3: False,
        }  # Track left and right mouse buttons
        ...
        for entity_type, path in entity_sprite_paths.items():
            try:
                ...
                logger.warning(
                    f"Failed to load entity sprite {path} for {entity_type}: {e}"
                )
    ...
    def initialize(self):
        ...
        try:
            ...
            logger.info(
                f"Player state fetched: energy={self.player_data.get('energy')}, direction={self.player_data.get('direction')}"
            )
        ...
        for attempt in range(1, max_retries + 1):
            try:
                logger.debug(
                    f"Loading initial room (0, 0), attempt {attempt}/{max_retries}"
                )
                self.load_room(0, 0)
                break
            except Exception as e:
                logger.error(
                    f"Failed to load initial room on attempt {attempt}: {e}",
                    exc_info=True,
                )
                ...
                if attempt < max_retries:
                    ...
                    self.status.set(
                        f"Room load failed (attempt {attempt}/{max_retries}), retrying..."
                    )
                    ...
                    self.status.set(
                        f"Room load failed after {max_retries} attempts: {e}"
                    )
    ...
    def _predictPosition(self, direction: int):
        ...
        if (
            self.current_room
            and current_room_x == self.current_room_x
            and current_room_y == self.current_room_y
        ):
            ...
            if (
                predicted_tile_x < 0
                or predicted_tile_x >= width
                or predicted_tile_y < 0
                or predicted_tile_y >= height
            ):
                logger.debug(
                    f"Prediction blocked - out of bounds: ({predicted_tile_x}, {predicted_tile_y})"
                )
            ...
            for entity in entities:
                ...
                if location_id:
                    ...
                    if len(parts) >= 4:
                        ...
                        if (
                            entity_tile_x == predicted_tile_x
                            and entity_tile_y == predicted_tile_y
                        ):
                            if entity.get("solid", False):
                                logger.debug(
                                    f"Prediction blocked - solid entity at ({predicted_tile_x}, {predicted_tile_y})"
                                )
                                return (current_tile_x, current_tile_y)
    ...
    def _updateInventoryFromServerData(self, inventory_data):
        ...
        logger.info(
            f"Syncing inventory: {len(slots_data)} slots, {inventory_data.get('numItems', 0)} total items"
        )
        ...
        logger.info(
            f"Inventory sync complete: {self.player.getInventory().getNumItems()} items restored"
        )
    ...
    def movePlayer(self, direction: int):
        ...
        logger.debug(
            f"movePlayer called: direction={direction} ({direction_names[direction]})"
        )
    ...
    def toggleGathering(self):
        ...
        try:
            ...
            logger.debug(
                f"Current gathering state: {is_gathering}, toggling to: {not is_gathering}"
            )
            self.player_data = self.api_client.perform_player_action(
                "gather", gathering=not is_gathering
            )
            logger.info(
                f"Gathering toggle successful: now gathering={not is_gathering}"
            )
            self._updatePlayerFromServerData(self.player_data)
    ...
    def handleKeyDownEvent(self, key):
        ...
        # Regular arrow key movement
            ...
            try:
                ...
                self.player_data = self.api_client.perform_player_action(
                    "crouch", crouching=not current_crouching
                )
                self._updatePlayerFromServerData(self.player_data)
            ...
            for i, slot in enumerate(slots):
                if not slot.get("empty", True):
                    ...
                    if item_name:
                        ...
                        logger.debug(
                            f"Calling API: perform_player_action('consume', item_name={item_name})"
                        )
                        self.player_data = self.api_client.perform_player_action(
                            "consume", item_name=item_name
                        )
    ...
    def _performGatherAt(self, screen_x: int, screen_y: int):
        ...
        if tile_coords:
            ...
            try:
                self.player_data = self.api_client.perform_player_action(
                    "gather", gathering=True, tile_x=tile_x, tile_y=tile_y
                )
                self._updatePlayerFromServerData(self.player_data)
    ...
    def _performPlaceAt(self, screen_x: int, screen_y: int):
        ...
        if tile_coords:
            ...
            try:
                self.player_data = self.api_client.perform_player_action(
                    "place", placing=True, tile_x=tile_x, tile_y=tile_y
                )
                self._updatePlayerFromServerData(self.player_data)
                if not self.config.debug:
    ...
    def _navigate_to_room(self, new_x: int, new_y: int, direction: str):
        ...
        if not self.load_room(new_x, new_y):
            logger.error(
                f"Failed to load room at ({new_x}, {new_y}) when navigating {direction}"
            )
    ...
    def load_room(self, room_x: int, room_y: int) -> bool:
        ...
        try:
            ...
            if not room:
                raise ValueError(
                    f"Server returned no data for room ({room_x}, {room_y})"
                )
            ...
            logger.debug(
                f"Room loaded successfully: {len(self.current_room.get('tiles', []))} tiles"
            )
            ...
            logger.error(
                f"Failed to load room ({room_x}, {room_y}): {e}", exc_info=True
            )
    ...
    def _hex_to_rgb(self, hex_color: str) -> tuple:
        ...
        if len(hex_color) != 6:
            logger.warning(
                f"Invalid hex color length '{hex_color}' (expected 6 characters)"
            )
    ...
    def render_world(self):
        ...
        for tile_data in tiles:
            ...
            if biome == "Grassland" and "Grass" in self.entity_sprites:
                self.graphik.getGameDisplay().blit(
                    self.entity_sprites["Grass"], (screen_x, screen_y)
                )
            else:
                ...
                if biome_color_hex:
                    ...
                    color = self.biome_colors_fallback.get(
                        biome, self.unknown_biome_color
                    )
        ...
        for entity in entities:
            ...
            if len(parts) >= 4:
                ...
                if entity_type in self.entity_sprites:
                    self.graphik.getGameDisplay().blit(
                        self.entity_sprites[entity_type], (screen_x, screen_y)
                    )
                else:
                    # Log unknown entity types explicitly
                    if entity_type not in ["", "Unknown"]:
                        logger.warning(
                            f"Unknown entity type encountered: '{entity_type}' (name: '{entity_name}'). Using colored square fallback. Consider adding sprite mapping."
                        )
        ...
        if self.player_data:
            ...
            if (
                player_room_x == self.current_room_x
                and player_room_y == self.current_room_y
            ):
                ...
                last_direction = self.player_data.get(
                    "lastDirection", 2
                )  # Default to down
                ...
                if display_direction in self.player_sprites:
                    self.graphik.getGameDisplay().blit(
                        self.player_sprites[display_direction], (screen_x, screen_y)
                    )
        ...
        self.graphik.getGameDisplay().blit(
            room_label, (world_view_x, world_view_y - 25)
        )
    ...
    def _drawInventoryPreview(self):
        ...
        start_y = (
            display_height - 100
        )  # Moved from -80 to -100 to avoid energy bar overlap
        ...
        pygame.draw.rect(
            ...
            (
                start_x - bar_padding,
                start_y - bar_padding,
                total_width + bar_padding * 2,
                slot_size + bar_padding * 2,
            ),
        )
        ...
        for i, slot in enumerate(slots):
            ...
            pygame.draw.rect(
                self.graphik.getGameDisplay(), color, (x, start_y, slot_size, slot_size)
            )
            ...
            if not slot.get("empty", True):
                ...
                self.graphik.getGameDisplay().blit(
                    count_surface,
                    (
                        x + (slot_size - text_width) // 2,
                        start_y + (slot_size - text_height) // 2,
                    ),
                )
    ...
    def run(self):
        ...
        tick_counter = 0
        tick_update_frequency = (
            20  # Update server tick every 20 frames (~3 ticks/sec at 60 FPS)
        )
