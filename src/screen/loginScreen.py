...
class LoginScreen:
    ...
    def drawForm(self):
        ...
        username_y = start_y
        username_color = (
            (255, 255, 100) if self.active_field == "username" else (200, 200, 200)
        )
        # Draw label above field to avoid overlap
        self.graphik.drawText(
            "Username:", field_x + field_width / 2, username_y - 20, 18, (200, 200, 200)
        )
        pygame.draw.rect(
        ...
        password_color = (
            (255, 255, 100) if self.active_field == "password" else (200, 200, 200)
        )
        # Draw label above field to avoid overlap
        password_label = (
            "Password: (Press P to show)"
            if not self.show_password
            else "Password: (Press P to hide)"
        )
        self.graphik.drawText(
            password_label,
        ...
        if self.show_password:
            ...
            display_password = (
                "*" * len(self.password_input) if self.password_input else ""
            )
        self.graphik.drawText(
            display_password, field_x + 10, password_y + 25, 20, (255, 255, 255)
        )
        ...
        if self.show_registration:
            ...
            email_color = (
                (255, 255, 100) if self.active_field == "email" else (200, 200, 200)
            )
            # Draw label above field to avoid overlap
            self.graphik.drawText(
                "Email:", field_x + field_width / 2, email_y - 20, 18, (200, 200, 200)
            )
            pygame.draw.rect(
                self.graphik.getGameDisplay(),
                email_color,
            ...
            self.graphik.drawText(
                self.email_input or "", field_x + 10, email_y + 25, 20, (255, 255, 255)
            )
        ...
        if self.error_message:
            self.graphik.drawText(
                self.error_message, x / 2, message_y, 18, (255, 50, 50)
            )
        if self.success_message:
            self.graphik.drawText(
                self.success_message, x / 2, message_y, 18, (50, 255, 50)
            )
        ...
        toggle_text = (
            "Already have an account? Press R to Login"
            if self.show_registration
            else "Don't have an account? Press R to Register"
        )
