...
class InventoryScreen:
    ...
    def _syncInventoryFromServer(self):
        ...
        try:
            ...
            print(
                f"DEBUG: Syncing inventory from server. Got {len(inventory_data.get('slots', []))} slots"
            )
            ...
            for slot_index, slot_data in enumerate(slots_data):
                ...
                print(
                    f"DEBUG: Slot {slot_index}: itemName={item_name}, numItems={num_items}, empty={slot_data.get('empty')}"
                )
                ...
                print(
                    f"DEBUG: Added {num_items} {item_name}(s) to slot {slot_index}. Slot now has {inventory_slot.getNumItems()} items"
                )
            ...
            print(
                f"DEBUG: Inventory sync complete. Total items in inventory: {self.inventory.getNumItems()}"
            )
        ...
        if self.cursorSlot.isEmpty():
            ...
            if self.api_client and self.session_id and self.from_slot_index is not None:
                try:
                    ...
                    self.api_client.swap_inventory_slots(
                        self.from_slot_index, index, self.session_id
                    )
            ...
            if (
                ...
                if self.cursorSlot.isEmpty():
                    ...
                    if (
                        self.api_client
                        and self.session_id
                        and self.from_slot_index is not None
                    ):
                        try:
