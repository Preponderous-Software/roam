...
- **`src/inventory/inventoryJsonReaderWriter.py` (`saveInventory`), `src/codex/codexJsonReaderWriter.py` (`save`):** Both writers validated the serialized data against their JSON schema, caught the `ValidationError`, **logged it, and then wrote the invalid data anyway** — so validation offered no protection. Both now `return` on a validation failure (after an explanatory log), leaving the existing good file untouched. Because validation runs before the `open(path, "w")` truncation, aborting preserves the prior save byte-for-byte.
- **`tests/inventory/test_inventoryJsonReaderWriter.py`, `tests/codex/test_codexJsonReaderWriter.py` (new):** Added regression tests that force a `ValidationError` (monkeypatching `jsonschema.validate`) and assert the existing file is not clobbered and no new file is created on failure; a codex save/load round-trip test was also added. All three abort tests were confirmed to **fail without the fix** (the old code logged the error and then re-wrote the file).
- **Validation:** `python3 -m compileall src -q` clean; `SDL_VIDEODRIVER=dummy SDL_AUDIODRIVER=dummy python3 -m pytest` — 468 passed (was 464; +4 new).
- **Scope note:** Deliberately limited to the two writers #363 names. The broader atomic-write + guarded-load rework (#370) — temp-file + `os.replace`, and resilient `json.load` across all serializers — is left as its own follow-up; the abort-on-invalid policy here is the minimal fix for the "invalid data still written" defect.
- **Design tradeoff (for human review):** Aborting on validation failure converts a best-effort save into strict-or-nothing. If a schema were ever too strict, a legitimate save could be silently dropped on exit. This is the behavior #363 requests, but it is a save-policy change on player-data serializers, so this PR is left for maintainer review rather than auto-merged.
- **Learning Log:** `[integrated]` — applied cycle-4 hunk-scoping: file-scoped Black reformatted a pre-existing-drift block in the existing `test_saveInventory`, which was reverted so the diff carries only the intended additions.
