...
- **Two deliberate exceptions:** `-psn_<n>_<n>` is skipped rather than rejected, because Finder can hand that argument to a launched macOS `.app` bundle and the repo ships one (`macos-package.yml`); and `--web` remains an accepted flag but is left out of `_USAGE`, since #550 reports it broken end to end and the supported browser path is `web/serve.py`. A test asserts the usage text does not mention `--web`, so resolving #550 in favour of repairing the mode will surface this as a failing test rather than as forgotten documentation.
