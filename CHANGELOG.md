...
|------|---------|---------|
| 2026-09-26 | 1 | fix: Binary save files (map PNGs) are stored in IndexedDB as raw bytes — `makeSyncSaves` read every file with `FS.readFile(path, {encoding: 'utf8'})`, which never throws and stops at the first NUL byte, so `mapImage.png` was persisted as a ~6-character string and restored unreadable. Files that decode as strict UTF-8 (the JSON saves) are still stored as strings, exactly as before; anything else is stored as an `ArrayBuffer`, which the restore path already handled. No DB name/store/version change; existing string records restore unchanged. `test_save_persistence.js` now seeds a real PNG and asserts it round-trips IDB → FS → IDB byte-identically; new `test_legacy_save_records.js` seeds pre-fix string records (including the mangled PNG) and asserts the save loads with its JSON intact |
| 2026-09-26 | 1 | test: Characterization coverage for `ProgrammaticBehavior`, the default NPC decision loop — `tests/npc/test_programmaticBehavior.py` covered eating, gathering, placing and the oscillation guard, but resource seeking, exit seeking, the shared `_npcMove` fallback (also used by `AgenticBehavior`), `_RoomKnowledge` and the grid helpers had no test, and `programmaticBehavior.py` sat at 69% line coverage. 54 tests added. Writing them surfaced #577: `OakWood` and `Stone` are solid and gathering is same-tile only, so a programmatic NPC never gathers the wood or stone it seeks — pinned by a characterization test, not fixed. Four mutations of `programmaticBehavior.py` each failed the tests written for them. No production code changed. +54 tests; 1449 passed locally; `programmaticBehavior.py` coverage 69% → 99% |
| 2026-09-25 | 1 | test: Characterization coverage for `NpcManager`'s room crossing, tick dispatch and behavior selection — `tests/npc/test_npcManager.py` covered mode toggling, spawning, death drops and cleanup, but `_handleRoomCrossing` (the only code that moves an NPC between rooms) had no test and `npcManager.py` sat at 63% line coverage. 25 tests added: each of the four edge crossings lands on the opposite edge of the correct neighbouring room, corners, interior tiles, unloaded target rooms and stale location IDs are all refused with the NPC left in place; `tickActiveRooms` requests exactly the `(2r+1)²` rooms around the player at the given `z`, skips unloaded rooms and non-`Npc` living entities, forwards the tick, and reports the source room as dirty on any exit request and the target room only when the crossing succeeds; `_getBehavior` picks `AgenticBehavior` when either the NPC or the manager is in CPC mode and caches per NPC; and spawning seeds three oak wood and three apples, prefers a non-solid tile, and falls back to a random tile when every tile is solid. Four mutations of `npcManager.py` each failed only the tests written for them. No production code changed. +25 tests; 1395 passed locally; `npcManager.py` coverage 63% → 99% |
| 2026-09-21 | 1 | test: Characterization coverage for the DI wiring in `src/bootstrap.py` — `createContainer` was the one piece of infrastructure in the tree with no test of its own: `tests/di/test_container.py` proves the container's generic lifetimes and auto-wiring, and `tests/conftest.py` calls `createContainer` to build every test's container, but nothing asserted *what* it registers. 24 tests added in `tests/test_bootstrap.py` pinning the choices the rest of the game depends on: `RoomFactory`, `RoomJsonReaderWriter` and `Map` are transient while `Player`, `RoomPreloader` and `LoggerFactory` are singletons; each factory pulls `gridSize` from the registered `Config` and shares the one `Renderer` and `TickCounter`; `Player` is built from the tick counter's *current* tick; each `Map` gets its own `RoomFactory` and both `Map` and `RoomPreloader` receive a `roomJsonReaderWriterFactory` yielding a fresh writer per call; and a second `createContainer` call — what `Roam.restart()` does — swaps in the new `Config`, drops the cached `@component` singletons via `resetSingletons()`, replaces the factory-registered ones by re-registration, and leaves explicit `registerInstance` entries such as the `Renderer` untouched. Three deliberate mutations of `bootstrap.py` (making `RoomFactory` a singleton, removing `resetSingletons()`, constructing `Player(0)`) each failed only the tests written for that behaviour before being reverted. No production code changed. +24 tests; 1370 passed locally |
| 2026-09-15 | 1 | docs+feat: Disclose usage reporting: README, environment opt-out, notice link — `src/lib/trace_client.py` re-vendored from trace-client-python 0.2.0 (with its upstream test), whose constructor honours `TRACE_USAGE_REPORTING=off` / `DO_NOT_TRACK=1` before anything else and exposes `disabled_reason`; `usageReporting.isDisabledByEnvironment()` consults the same two variables through the client's `environment_opts_out()` next to `ROAM_USAGE_REPORTING=0`, so the first-run notice is never logged (and the marker never written) for a run that will not report. The notice now carries the details link (https://github.com/Stephenson-Software/trace#usage-reporting) and names `TRACE_USAGE_REPORTING=off`; the README section is promoted to a top-level `## Usage reporting` listing both events, what is not sent, every opt-out and that the Pyodide build is silent; `config.yml`'s comment block names the variables and the link. Nothing about what is sent changed. +16 tests; 1364 passed locally |
...
### 2026-09-26 — Characterization coverage for `ProgrammaticBehavior`
- **Context:** the open backlog was unchanged from 2026-09-25 and every issue was still blocked for the reasons recorded there. #556 and #550 each wait on a design decision, and #551 edits both agent-loaded instruction files. A Stage-B unit-test expansion was run instead. A `pytest --cov=src` pass put `src/npc/programmaticBehavior.py` at 69% line coverage, the lowest of the `npc` package after the previous two sessions. The missed lines covered all of priority 4 (seek nearest resource), all of priority 5 (seek a room exit), the inventory-full wander branch, `_RoomKnowledge`, `_directionToward`, and the blocked-move fallback in `_npcMove`.
- **Why this module:** `ProgrammaticBehavior` is the default NPC mode, and `_locationOf` / `_npcMove` are imported by `AgenticBehavior` and `NpcManager`. A change to the fallback order or the edge handling would therefore change both NPC modes without failing any test.
- **Tests (+54):** in `tests/npc/test_programmaticBehavior.py`.
  - Helpers: `_locationOf` returns `None` for the `"-1"` sentinel and for an ID not in the grid, and `tick` then takes no action. `_getNeighbor` maps the four direction codes and returns `None` past each edge. `_directionToward` breaks ties toward horizontal, and for the same tile it returns "left".
  - `_npcMove`: a blocked step tries the clockwise perpendicular first, then the counter-clockwise one, then reverse only when `allowReverse`. A fully blocked move consumes the turn without spending energy.
  - `_RoomKnowledge`: unknown rooms are assumed to have both resources, and `bestExitDirection` picks the nearest matching room. A matching record for the current room points "left".
  - Priorities: hungry NPCs do not eat non-food items. A living entity on the NPC's own tile is not gathered. A blocked placement falls through to lower priorities. A full inventory of `Stone` wanders with the "inventory full" goal, while a full inventory of `OakWood` builds first. A hungry NPC walks to visible food and eats it. An existing target is not replaced on rescan, and `_findNearest` skips blacklisted targets. A hungry NPC in an empty room walks toward the nearest edge, requests a room change at the edge, prefers the direction of a room known to have food, and waits when boxed in. The wander direction is held for `_WANDER_STEPS` picks.
- **Bug found, not fixed:** `test_npc_sidesteps_solid_wood_instead_of_stepping_onto_it` pins current behavior. `OakWood` and `Stone` are both solid, `_npcMove` never enters a solid tile, and `_tryGather` reads only the NPC's own tile, so a non-hungry NPC never gathers the room's wood or stone. The build-shelter priority is unreachable in normal play as a result. This was filed as #577 rather than changed under a test-expansion cycle.
- **Not vacuous:** four mutations were applied to `src/npc/programmaticBehavior.py` together, and the file was re-run. They were: swapping the perpendicular fallback order in `_npcMove`, dropping the room-change flag at an edge, appending instead of prepending the preferred exit direction, and removing the blocked-target skip in `_findNearest`. Six tests failed, each one written for the mutation that broke it. The source was restored from `git` and the suite reconfirmed green. Lines 137 (the wood filter in `bestExitDirection`, which `tick` only ever calls with `wantFood=True`) and 368 (`woodSlot.pop()` returning `-1` right after a non-empty check) remain uncovered because production code cannot reach them.
- **Validation:** 1449 passed via `python3 -m pytest -q --ignore=tests/mapimage/test_mapImageGenerator.py`, up from 1395. That file needs Python 3.10+ parenthesized-context-manager syntax, which this sandbox's Python 3.8.10 cannot collect. The exclusion is pre-existing and unrelated, and CI runs a current Python with no exclusion. The new file was run three times to check for flakiness under the module's `random` calls. `python3 -m black --check src tests` was clean on 23.1.0. Only test code and `CHANGELOG.md` changed. Nothing under `src/` changed, and no frontend-abstraction surface is involved.

### 2026-09-25 — Characterization coverage for `NpcManager` room crossing and tick dispatch
- **Context:** every open issue at triage time was still blocked or out of polish scope, for the reasons recorded on 2026-09-21 — #393/#396 need paid code-signing identities, #415/#416 are gated on those two, #234 needs a live Viron server, #556 asks three unanswered design questions, #550 states two mutually exclusive resolutions and asks which is intended, and #551 edits both agent-loaded instruction files, which require separate human authorization. A Stage-B unit-test expansion was run instead. A `pytest --cov=src` pass put `src/npc/npcManager.py` at 63% line coverage, and every missed line was in behaviour-bearing code: `_handleRoomCrossing` in its entirety, the room-change branch of `tickActiveRooms`, the CPC branch of `_getBehavior`, and the two fallbacks in spawning.
- **Why this module:** `_handleRoomCrossing` is the only code that moves an NPC between rooms, and its result decides which rooms `tickActiveRooms` reports as dirty — the set `WorldScreen` saves asynchronously. A swapped edge coordinate or a lost dirty flag would raise nothing; it would show up as NPCs appearing on the wrong side of a room, or vanishing from a room that was never re-saved.
- **Tests (+25):** in `tests/npc/test_npcManager.py`. Room crossing: each of the four edges lands on the opposite edge of the correct neighbouring room with the other coordinate preserved; the NPC is removed from the source room's grid and living-entity map; corners, interior tiles, an unloaded target room and a location ID absent from the grid are all refused with the NPC left in place. Tick dispatch: exactly the `(2r+1)²` rooms around the given coordinates are requested at the given `z`; unloaded rooms and non-`Npc` living entities are skipped; the tick is forwarded to the behavior; the source room is dirty on any exit request and the target only when the crossing succeeds; `clearRoomChangeRequest` is called once. Behavior selection: `ProgrammaticBehavior` in NPC mode, `AgenticBehavior` when either the NPC or the manager is in CPC mode, cached per NPC. Spawning: three oak wood and three apples seeded; a non-solid tile preferred; `_findOpenSpawnLocation` returns `None` on an all-solid room and `spawnNpc` then falls back to a random (solid) tile rather than failing; death drops are a no-op for an NPC not on the grid.
- **Test doubles:** behaviors are replaced by a `_StubBehavior` injected into `_behaviors`, so the manager's own dispatch logic runs without the FSM's randomness deciding whether an exit is requested; rooms are real `Room` objects behind a `_FakeMap` that records which coordinates were requested.
- **Not vacuous:** four mutations were applied to `src/npc/npcManager.py` together and the file re-run — disabling the corner check, sending an east-edge crossing to the east edge instead of the west, dropping the NPC's own mode from the CPC test in `_getBehavior`, and dropping the source-room dirty flag. Five tests failed, each one written for the mutation that broke it; the source was restored from `git` and the file reconfirmed green. Line 142 (`targetLoc == -1`) remains uncovered: with every room built from the same `gridSize`, the opposite-edge coordinate always exists.
- **Validation:** 1395 passed via `python3 -m pytest -q --ignore=tests/mapimage/test_mapImageGenerator.py` (up from 1370; that file needs Python 3.10+ parenthesized-context-manager syntax this sandbox's Python 3.8.10 cannot collect — pre-existing and unrelated, and CI runs a current Python with no exclusion). `python3 -m black --check src tests` clean on 23.1.0. Test code and `CHANGELOG.md` only; nothing under `src/` changed, and no frontend-abstraction surface is involved.

### 2026-09-21 — Characterization coverage for the DI wiring in `bootstrap.py`
- **Context:** every open issue at triage time was blocked or out of polish scope — #393/#396 need paid code-signing identities, #415/#416 are gated on those two, #234 needs a live Viron server, #556 asks three unanswered design questions, #550 states two mutually exclusive resolutions and asks which is intended, and #551 edits both agent-loaded instruction files, which require separate human authorization. A documentation sweep found README's controls table, `PLANNING.md`'s recipe/room/mob lists, `LOGGING.md`'s env vars and every `config.yml` key still accurate against source, so a Stage-B unit-test expansion was run instead. Comparing `src/` against `tests/` left `src/bootstrap.py` as the only infrastructure module with no mirror: `tests/di/test_container.py` covers the container's generic behaviour, and `tests/conftest.py` *calls* `createContainer` for every test, but nothing asserted what it registers.
- **Why this module:** the choices it makes are load-bearing and invisible. The `CHANGELOG` Learning Log already records the trap — `Map`, `RoomFactory` and `RoomJsonReaderWriter` are deliberately transient, so constructor-injecting one into a new singleton component silently yields a disconnected instance rather than the one `WorldScreen` holds — and `.github/copilot-instructions.md`'s "Restart Safety" section documents that `Roam.restart()` relies on `createContainer` clearing cached singletons. A lifetime flipped by accident, or the `resetSingletons()` call dropped, would break neither the unit suite nor `--selftest`; it would surface as stale state after an in-game restart.
- **Tests (+24):** `tests/test_bootstrap.py`. What `createContainer` hands back and registers outright (the module-level container, `Container` as itself, the exact `Config` instance, a singleton `LoggerFactory`); `Player` built from `TickCounter.getTick()` at resolve time and cached; `RoomFactory`, `RoomJsonReaderWriter` and `Map` each transient and each wired from `Config.gridSize`, the registered `Renderer` and the singleton `TickCounter`; `Map` receiving a container-built `RoomFactory` of its own per instance, and both `Map` and `RoomPreloader` receiving a `roomJsonReaderWriterFactory` that yields a fresh writer per call; `RoomPreloader` a singleton; and restart safety — a second `createContainer` swaps in the new `Config` (a `RoomFactory` resolved afterwards sees the new `gridSize`), drops the cached `@component` singleton (`TickCounter`), drops the factory-registered singletons (`Player`, `RoomPreloader`), replaces the `LoggerFactory`, preserves the explicitly registered `Renderer`, and a `Player` resolved after the restart reads the *new* tick counter.
- **Two reset mechanisms, not one:** writing these separated something the docstring on `createContainer` folds together. `resetSingletons()` only matters for types registered by `@component` at import time (`TickCounter`), because `createContainer` never re-registers those. Every type `createContainer` registers itself (`Player`, `RoomPreloader`, the transients) is refreshed by the re-registration alone — `Container.register` installs a new `_Registration` with no cached instance. The tests are split along that line so a future change to either mechanism fails the test that names it.
- **Not vacuous:** three mutations were applied to `src/bootstrap.py` and the file re-run. Dropping `lifetime="transient"` from the `RoomFactory` registration failed exactly `test_room_factory_is_transient` and `test_each_map_gets_its_own_room_factory`; deleting the `resetSingletons()` call failed exactly the component-singleton restart test (and *only* that one — the first evidence of the two-mechanism split above); replacing the `Player` factory with `Player(0)` failed exactly the two tick-wiring tests. The source was restored from `git` after each and the file reconfirmed green.
- **Validation:** 1370 passed via `python3 -m pytest -q --ignore=tests/mapimage/test_mapImageGenerator.py` (up from 1346; that file needs Python 3.10+ parenthesized-context-manager syntax this sandbox's Python 3.8.10 cannot collect — pre-existing and unrelated, and CI runs a current Python with no exclusion). `python3 -m black --check src tests` clean on 23.1.0, the version CI pins. Test code and `CHANGELOG.md` only; nothing under `src/` changed, and no frontend-abstraction surface is involved.
- **Learning Log:** `[not yet integrated]` — a "reset" that a restart path relies on can be doing less than its docstring claims without any test noticing, because re-registration on the same code path masks it for most types. When characterizing a bootstrap/composition-root function, mutate each cleanup call separately and record which types each one actually protects — here `resetSingletons()` guards only the `@component` types, and a future `@component` added to `bootstrap.py`'s neighbourhood is the case that would regress if it were removed.
