...
|------|---------|---------|
| 2026-06-07 | 1+ | ci: Smoke-test the Windows installer on CI — add `-NonInteractive`/`-NoLaunch` switches to `install.ps1` (suppress prompts and skip launch for automation) and a `windows-latest` GitHub Actions job that runs the wizard non-interactively, asserts it generated `icon.ico` plus Desktop/Start Menu shortcuts, and runs the test suite on Windows |
