...
|------|---------|---------|
| 2026-04-19 | 12 | fix: Track player-visited rooms with a `visitedRooms` set so the 'rooms explored' statistic increments whenever the player enters a room for the first time — rooms pre-generated by rendering or entity movement no longer silently swallow the count; fix: Persist `visitedRooms` to `visitedRooms.json` between sessions so rooms are not double-counted after save/load; fix: Use `with` context managers in save/load for proper file handle cleanup; fix: Only persist `visitedRooms` when set has changed (dirty flag); fix: Living entity room generation now uses entity coordinates instead of player direction; fix: Legacy saves without `visitedRooms.json` auto-migrate by scanning existing room files; feat: Add lightweight DI container (`src/di/`) with auto-wiring, singleton/transient lifetimes, `@component` decorator, circular dependency detection, and factory function support; feat: Create container singleton (`src/appContainer.py`) and centralized bootstrap (`src/bootstrap.py`); refactor: Migrate `roam.py`, `worldScreen.py`, `map.py` to resolve dependencies via container; fix: Remove dead EnergyBar fallback branch in WorldScreen; fix: Add `resetSingletons()` to prevent stale cached instances across game restarts; refactor: Replace new-instance restart with `Roam.restart()` method that resets state on the existing instance; docs: Document DI strategy in `copilot-instructions.md` for contributors; test: Add 15 DI test cases |
...
- 2026-04-19: `[integrated]` The `getOrLoadRoom` method in
  ...
  player-explored rooms must not rely on room generation as a proxy;
  instead, use the `visitedRooms` set introduced in this session to
  determine whether a room has been entered by the player.
