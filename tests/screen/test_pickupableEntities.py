...
@pytest.mark.parametrize("oreClass", (CoalOre, IronOre, GoldOre))
def test_every_ore_can_be_picked_up(oreClass):
    # All three ores are generated by cave rooms, so all three must be
    # gatherable — gold was spawned but unpickupable at one point.
    assert canBePickedUp(oreClass()) is True


def test_empty_iron_chest_can_be_picked_up():
    # IronChest subclasses Chest, so it must inherit the empty-only rule.
    assert canBePickedUp(IronChest()) is True


def test_non_empty_iron_chest_cannot_be_picked_up():
    chest = IronChest()
    chest.getStoredInventory().placeIntoFirstAvailableInventorySlot(Apple())
...
def test_golden_lantern_can_be_picked_up():
    assert canBePickedUp(GoldenLantern()) is True
